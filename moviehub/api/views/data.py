...
from moviehub.core.models import AuthToken

@app.route("/_data/setup/")
def add_trusted_client():
    from moviehub.core.models import Client, User, Movie

    # user and clients
    # ----------------
    user = User(email="mikaelahlen+1@gmail.com",
    ...
    client = Client(redirect_uri="https://movie-hub.appspot.com/auth/", name="Moviehub", trusted=True, user=user)
    client.generate_secret()
    client.put()

    token = AuthToken.generate_token(user, client)

    output = "Frontend setup instructions\n<br>"
    output += "--------------------------\n<br>"
    output += "In frontend/__init__.py set the client_id and client_secret to\n<br>"
    output += "client_id=%s\n<br>client_secret=%s\n\n<br><br>" % (client.key().id(), client.secret)
    ...
    for i, m in enumerate(movies):
        if i % 2 == 0:
            m.likes.append(user.key())
        m.put()

    output += "Added movies\n<br>"

    output += "To add recommendations/reasons: <br>\n"
    output += "-------------------------------"
    output += "For localhost: use the frontend port, not the api because it calls itself (the api)<br>\n"
    output += "/_data/add/recommendations/?id=%s&secret=%s&token=%s" % (
        client.key().id(), client.secret, token.token)
...
def add_recommendation_data():
    ...
    p = Moviehub(client_id=request.args.get("id"), client_secret=request.args.get("secret"),
